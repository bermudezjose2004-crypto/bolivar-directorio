...
const ACTIVO_ICONS: Record<string, string> = {
...
const INITIAL = [
  { id:1, nombre:"Valentina Rios", puesto:"Gerente General", depto:"Operaciones", email:"valentina.rios@bolivar.com", activo:true, activos:["Laptop","Cargador laptop","Llave parqueo","Tarjeta acceso"], devueltos:[] as string[] },
  { id:2, nombre:"Andres Molina", puesto:"Director de Finanzas", depto:"Finanzas", email:"andres.molina@bolivar.com", activo:true, activos:["Laptop","Cargador laptop","Monitor"], devueltos:[] as string[] },
  { id:3, nombre:"Camila Herrera", puesto:"Coordinadora de Marketing", depto:"Marketing", email:"camila.herrera@bolivar.com", activo:true, activos:["Laptop","Telefono","Tarjeta acceso"], devueltos:[] as string[] },
  { id:4, nombre:"Luis Paredes", puesto:"Desarrollador Senior", depto:"Tecnologia", email:"luis.paredes@bolivar.com", activo:true, activos:["Laptop","Cargador laptop","Monitor","Mouse","Teclado"], devueltos:[] as string[] },
  { id:5, nombre:"Sofia Guerrero", puesto:"Analista de RRHH", depto:"RRHH", email:"sofia.guerrero@bolivar.com", activo:true, activos:["Laptop","Cargador laptop"], devueltos:[] as string[] },
  { id:6, nombre:"Mateo Castillo", puesto:"Contador", depto:"Finanzas", email:"mateo.castillo@bolivar.com", activo:true, activos:["Laptop","Monitor","Llave parqueo"], devueltos:[] as string[] },
  { id:7, nombre:"Isabella Vega", puesto:"Disenadora UX", depto:"Tecnologia", email:"isabella.vega@bolivar.com", activo:true, activos:["Laptop","Cargador laptop","Mouse","Mochila"], devueltos:[] as string[] },
  { id:8, nombre:"Diego Ramirez", puesto:"Ejecutivo de Ventas", depto:"Operaciones", email:"diego.ramirez@bolivar.com", activo:true, activos:["Laptop","Telefono","Llave parqueo","Tarjeta acceso"], devueltos:[] as string[] },
  ];

function initials(n: string) { return n.split(" ").slice(0,2).map((w:string)=>w[0]).join("").toUpperCase(); }
...
export default function App() {
    ...
    const [editActivos, setEditActivos] = useState<number|null>(null);
    const [newEmp, setNewEmp] = useState({nombre:"",puesto:"",depto:"Operaciones",email:"",activos:[] as string[],devueltos:[] as string[]});
  ...
  function toggleDevuelto(empId: number, item: string) {
        setEmployees(prev=>prev.map(e=>{
                if(e.id!==empId) return e;
                const ya = e.devueltos.includes(item);
                return {...e, devueltos: ya ? e.devueltos.filter((d:string)=>d!==item) : [...e.devueltos, item]};
        }));
  }

  function toggleActivo(empId: number, item: string) {
        setEmployees(prev=>prev.map(e=>{
                if(e.id!==empId) return e;
                const has = e.activos.includes(item);
                return {...e, activos: has ? e.activos.filter((a:string)=>a!==item) : [...e.activos,item], devueltos: e.devueltos.filter((d:string)=>d!==item)};
        }));
  }

  function handleDeparture(emp: any) {
        ...
        <div style={{padding:"1.5rem",fontFamily:"sans-serif",background:"white",minHeight:"100vh"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:"1.25rem"}}>
                {[["Activos",activos.length],["Salidas",inactivos.length],["Departamentos",Object.keys(DEPT_COLORS).length]].map(([l,v])=>(
                    <div key={String(l)} style={{background:"rgba(240,240,240,0.97)",borderRadius:8,padding:"0.75rem 1rem"}}>
          ...
          {notifStatus && (
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {filtered.map(emp=>{
                    const dc = (DEPT_COLORS as any)[emp.depto]||{bg:"#F1EFE8",text:"#444441"};
                    ...
                    const pendientes = emp.activos.filter((a:string)=>!devueltos.includes(a));
                    return (
                                  <div key={emp.id} style={{background:"white",border:"1px solid #ddd",borderRadius:12,overflow:"hidden"}}>
                                                <div style={{padding:"0.85rem 1rem",display:"flex",alignItems:"center",gap:12,cursor:"pointer"}} onClick={()=>setExpanded(isOpen?null:emp.id)}>
                                                  ...
                                                  {emp.activos.length>0 && <span style={{fontSize:12,padding:"3px 8px",borderRadius:6,background:pendientes.length===0?"#E1F5EE":"#FAEEDA",color:pendientes.length===0?"#085041":"#633806",flexShrink:0}}>{devueltos.length}/{emp.activos.length}</span>span>}
                                                                <span style={{fontSize:16,color:"#666",flexShrink:0}}>{isOpen?"▲":"▼"}</span>span>
                                                </div>div>
                                    {isOpen && (
                                                    <div style={{borderTop:"1px solid #ddd",padding:"0.75rem 1rem",background:"#fafafa"}}>
                                                      ...
                                                      {isEditing ? (
                                                                          ...
                                                                          <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:"0.75rem"}}>
                                                                            {emp.activos.length===0 ? <span style={{fontSize:13,color:"#999"}}>Sin activos asignados</span>span> : emp.activos.map((item:string)=>{
          ...
          {showAdd && (
                  <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}}>
                            <div style={{background:"white",borderRadius:12,border:"1px solid #ddd",padding:"1.25rem",width:310}}>
                              ...
                              {([["Nombre completo","nombre","text"],["Puesto","puesto","text"],["Correo","email","email"]] as [string,string,string][]).map(([label,key,type])=>(
                                  <div key={key} style={{marginBottom:10}}>
                                                  <div style={{fontSize:12,color:"#666",marginBottom:4}}>{label}</div>div>
                                                  <input type={type} value={(newEmp as any)[key]} onChange={e=>setNewEmp(p=>({...p,[key]:e.target.value}))} style={{width:"100%"}}/>
                                  </div>div>
                                ))}
                                      ...
                                      return <button key={item} onClick={()=>setNewEmp(p=>({...p,activos:sel?p.activos.filter((a:string)=>a!==item):[...p.activos,item]}))} style={{fontSize:12,padding:"4px 10px",background:sel?"#F97316":"transparent",borderColor:sel?"#F97316":"#ccc",color:sel?"#fff":"#555"}}>{item}</button>button>;
                  })}
                                                      </div>div>
                                        </div>div>
                                        <div style={{display:"flex",justifyContent:"flex-end",gap:8}}>
